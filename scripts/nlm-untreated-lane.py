...
Two intake classes, both "untreated":
  add_failed  — fetched docs the mega-screen could not index into NotebookLM at all
                (50-source cap overflow, H-cycle 2026-08-25). NLM literally never saw them.
  unscreened  — fetched docs that were never in any screen queue (t10's tail).

Mechanics are the proven restage runner: 10 docs per DEDICATED follow-up notebook, in
series on the tab's own NLM account, every doc staged in full multi-part (F3c), the
broad question gating the consolidated one (an invented checklist is never credited),
per-chunk evidence JSON persisted, quota/notebook-cap backoff, slot-manager heartbeats.

Run INSIDE patent-bench:
  docker cp scripts/{nlm_followup.py,genus_maps.json,nlm-untreated-lane.py} patent-bench:/data/ &&
  docker exec -d patent-bench python3 /data/nlm-untreated-lane.py <tab> /data/audits/untreated_t<tab>.json <lane>
Log /data/.<lane>_t<tab>.log · progress /data/audits/<lane>_t<tab>.progress.json (idempotent re-arm).
Exit 2 (quota) from nlm_followup -> sleep 1h and retry the same chunk.
"""
...
def heartbeat(state, summary, **counts):
    try:
        ...
        tmp=f"{HB_DIR}/.patent-{LANE}-t{TAB}.tmp"
        with open(tmp,"w") as f:
            json.dump({"job":f"patent-bench {LANE} (genus wording) — tab {TAB}",
                       "account":PROFILE,"state":state,"summary":summary,
                       "counts":counts,
                       "updatedAt":time.strftime("%Y-%m-%dT%H:%M:%S.000Z",time.gmtime())},f)
        os.replace(tmp,f"{HB_DIR}/patent-{LANE}-t{TAB}.json")
    except Exception:
...
# body", t10 chunk 1 on 2026-08-29). Uncredited docs stay out of the progress file, but the
# single-pass loop used to walk past them and only a manual re-arm brought them back. Re-walk
# the uncredited remainder until a whole pass credits nothing new, so a dropped connection
# costs one retry instead of a silent hole in the corpus.
MAX_PASSES=4
for _pass in range(1, MAX_PASSES+1):
    ...
    before=len(done)
    if _pass>1:
        log(f"pass {_pass}: retrying {len(todo)} uncredited doc(s)")
    for i in range(0,len(todo),10):
        ...
        while True:
            ...
            break
    log(f"pass {_pass}: credited {len(done)-before} new doc(s) — {len(done)}/{len(docs)} total")
    if len(done)==before:
        log(f"pass {_pass} credited nothing new — stopping instead of spinning")
        break
heartbeat("done", f"finished — {len(done)}/{len(docs)} untreated docs assessed in full and credited",
