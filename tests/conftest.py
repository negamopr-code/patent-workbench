...
def client(tmp_path, monkeypatch):
    ...
    # every stub takes **k so the per-tab `profile=` kwarg (and future kwargs)
    # can never leak a call through to the real `nlm` binary
    monkeypatch.setattr(nlm_bridge, "query",
                        lambda nb, q, source_ids=None, **k: {"answer": f"nlm[{nb}]:{source_ids}",
                                                             "sources_used": []})
    monkeypatch.setattr(nlm_bridge, "list_notebooks",
                        lambda force=False, **k: {"notebooks": [{"id": "nb-1", "title": "NB", "sources": 2}]})
    monkeypatch.setattr(nlm_bridge, "list_sources",
                        lambda nb, force=False, **k: {"sources": [{"id": "s1", "title": "doc1"},
                                                                  {"id": "s2", "title": "doc2"}]})
    # the 🔬 mega-screen's staging path — stubbed so no test ever shells out to `nlm`
    monkeypatch.setattr(nlm_bridge, "add_source_text",
                        lambda nb, title, text, **k: {"ok": True})
    monkeypatch.setattr(nlm_bridge, "delete_source",
                        lambda ids, nb=None, **k: {"ok": True, "deleted": len(ids or [])})
    monkeypatch.setattr(nlm_bridge, "wait_sources_ready",
                        lambda nb, timeout=0, poll=0, known_ready=None, **k: {"ready": True,
                                                                              "processed": 0, "total": 0})
    monkeypatch.setattr(nlm_bridge, "source_content",
                        lambda sid, **k: {"content": "text"})
    monkeypatch.setattr(nlm_bridge, "create_notebook",
                        lambda title, **k: {"id": "nb-new", "title": title})
