  ...
  const buildRow = d => {
    ...
    if (d.score != null || d.nlm_score != null) {
      ...
      if (r) {
        ...
        if (d.legacy_wording) parts.push(`<span class="addl" title="Per-element reads were stored under an OLDER feature wording and re-keyed to the current names (by numeral-stripped name, position as fallback). The verdicts themselves are unchanged; re-read the document to assess it under the current wording.">⏳ old wording</span>`);
