...
| H3 | Stage-2 citation follow-up ≈ opus on graduates | **cycle 2 (nlm-followup-verifier 06:00–06:13, journal entry "2026-08-25 — nlm-followup-verifier"):** 7 R6 docs, all opus ≥4, all get ≥1 core-element YES with component citations at roster 3–4 — partial three-way table (v1 screen → stage-2 → blind opus): t11 CN223245862 rejected → 3×YES core (F3,F4,F5) → 5.0; CN115051084 rejected → 5×YES incl. weight-5 F1 (opus partial) → 4.0; CN220420731 rejected → F3,F4,F6,F7 YES → 4.0; CN223471682 rejected → F3,F4(opus NO),F6,F7 YES → 4.0; t13 CN206076985 graduate → F2 YES (w5) → 4.0; CN104760550 graduate → F4,F7,F8 YES → 4.0; CN224152886 rejected → F4–F8 YES → 4.0. Stage-2 agrees with opus on 7/7 at the champion level and is stricter/more generous on single features (3 weight-5 disagreements). Earlier ledgers (t10 5+10, t13 10 docs) still doc lists only; fu_t14_b1.json orphaned (08-24 21:51 restart) | SUPPORTED on 7/7 (small n; pre-audit); full table for the 25 earlier stage-2 docs still missing | 0 reads: opus column exists for all 32 stage-2 docs; needs the verifier's per-doc verdicts for the 25 |
| H4 | The t14 miss rate generalises to t11/t13 (default account) | t11 v2: one answered round (08-23 22:18: 19 grad / 19 rej, 25 add_failed), t13 v2: 0 rounds. **Correction (verifier probe 06:00):** the default account is NOT quota-blocked (answered in 10.6 s, quota=False); both screens sit at "waiting for NotebookLM to ingest the batch", round 0, error None since 08-23 = stalled ingest/watcher. Resuming is the user's call | OPEN (blocked on a stalled screen, not quota; no reads spent) | after the user resumes t11/t13: at ~150 v2-screened docs per tab, true-reject pool + 30-doc control. Note the verifier's finding that the v1 judged misses (H10) sat in roster-35 rounds → roster size (F3a), not doc property |
...
| H9 | t10's add_failed champions (6 docs, 08-17 verdicts) are stale too | cycle 2: US20220221016 6→5, US10996236 4, EP3849091 4, CA2552849 4 hold; TW201717523 4→3, CN113287245 4→3 | REFUTED (4/6 valid) → t10's staging champion loss is real; CLOSED 08-25 07:19: full pool read (97 more) found no further champion → exact t10 add_failed reject-miss count = 5 (all in the lane-selected part), 0 in the never-read 97 | none |
| H10 | **NEW (cycle 2, from recall-integrity audit).** The v2 screen has a non-zero JUDGED-miss rate (rejects an opus champion it actually saw) | Audit's judged misses are all v1-screen states: t11 CN223245862/CN115051084/CN220420731/CN223471682 screened 08-07/08-08; t12 KR20260033205/CN119833811 screened 08-06; t13 CN114690685/CN101639686 08-08/08-09 (truncated staging, NO-GO'd; roster-35 rounds → F3a). v2-screened rejects opus-read with ≥4: t10 0/96, t11 0/2, t12 0/9, t14 0/7 = **0/114**; ≥3: t10 0, t14 3/7. Registry (fc6e576) now carries F3c-ns (not-staged) per-tab baselines and R2-screen-roster39=39 — v2 runs roster 39 on all tabs, so the F3a corridor (≤12) is NOT met and a v2 judged-miss class may still appear once t11/t13 answer | SCOPED: 0/114 on v2 (pre-audit); v1 judged misses real but out of v2 scope | t11/t13 v2 rejects once the stalled screens are resumed (H4) |
...
- ~~t10: 97 unread add_failed docs~~ — APPROVED by user 08-25 ("read documents in t10 for clean up")
  and executed 07:03–07:19 UTC: 97/97 read, 0 ≥4, 2 at 3. Exact t10 add_failed reject-miss
  count = 5/173 (all in the lane-selected read part). Ledger: 11 jobs, 520 reads, proxy
  ≈ 53.1 M chars ≈ 13.3 M input tokens.
...
docs/experiments/read_ledger.jsonl (11 jobs, 520 reads, proxy ≈ 53.1 M chars ≈ 13.3 M
